...
export class DirectionComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.directionService
      ...
      .subscribe(
        (res: HttpResponse<IDirection[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInDirections(): void {
    this.eventSubscriber = this.eventManager.subscribe('directionListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IDirection[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(['/directioon/direction'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
